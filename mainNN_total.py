...
checkpoint_dir='/root/shiyan_total/model_total_model1/'
...
def main(use_cuda=True, EPOCHS=200, batch_size=8, model_name="avenet.pt"):
	...
	dataset = Mydata(img_speed_path='/user-data/lujianli/totaldata/train.txt',
    img_path='/user-data/lujianli/totaldata/train/img/',
    speed_path='/user-data/lujianli/totaldata/train/speed/')

	valdataset = Mydata(img_speed_path='/user-data/lujianli/totaldata/val.txt',
    img_path='/user-data/lujianli/totaldata/val/img/',
    speed_path='/user-data/lujianli/totaldata/val/speed/')
		...
		for epoch in range(EPOCHS):
			...
			for subepoch, (img, aud, out) in enumerate(dataloader):
				...
				if subepoch%400 == 0:
					...
					with open(file="./losses_total1.txt", mode="a+") as f:
			...
			for sepoch,(img, aud, out) in enumerate(valdataloader):
				...
				if sepoch%400==0:
					...
					with open(file="./vallosses_total1.txt", mode="a+") as f:
...
def test(use_cuda=True, batch_size=8, model_name="/root/shiyan_total/model_total_model/lowest_loss.pt"):
	model = getAVENet(use_cuda)
	if os.path.exists(model_name):
		model.load_state_dict(torch.load(model_name))
		print("Loading from previous checkpoint.")

	testdataset = Mydata(img_speed_path='/user-data/lujianli/totaldata/test.txt',
	 img_path='/user-data/lujianli/totaldata/test/img/',
	 speed_path='/user-data/lujianli/totaldata/test/speed/')
